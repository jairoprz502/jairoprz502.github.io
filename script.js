document.getElementById("jubilacionForm").addEventListener("submit", function(event) {
    ...
    const porcentajeTotal = porcentajeAnual * cantidadAniosLaborados; // Porcentaje total por todos los años de aporte
    const totalJubilacion = sueldoPromedio * porcentajeTotal; // Fórmula de jubilación

    if (cantidadAniosLaborados >= 20 || (edad > 60 && cantidadAniosLaborados >= 10)) {
        // Redirigir a la página de resultados con el total de jubilación calculado
